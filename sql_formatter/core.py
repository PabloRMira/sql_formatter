...
__all__ = ['MAIN_STATEMENTS', 'clean_query', 'preformat_statements', 'preformat_statements', 'lowercase_query',
           'format_partition_by', 'format_select', 'format_from', 'format_join', 'format_on', 'format_where',
           'format_statement_line', 'format_statements', 'add_ending_semicolon', 'format_simple_sql', 'format_sql']
...
def preformat_statements(s):
    ...
    split_s = split_comment_non_comment(s)  # split by comment and non comment
    for statement in statements:
        if re.match("create", statement):  # special case CREATE with AS capitalize as well
            split_c = [
                re.sub(rf"\s*({statement} )(.*) as\b", lambda pat: "\n" + pat.group(1).upper() + pat.group(2) + " AS", sp, flags=re.I)
                if comment == 1

                for sp, comment in split_s.items()
            ]
