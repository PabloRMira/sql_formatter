...
__all__ = ['assert_and_print', 'MAIN_STATEMENTS', 'remove_newlines_mspaces', 'preformat_statement', 'lowercase_query',
           'add_whitespaces_between_symbols', 'format_select_partition_by', 'format_select', 'format_partition_by',
...
def preformat_statement(s, statements):
    ...
    for line in split_s:  # loop over lines
        ...
        for statement in statements:  # loop over statements
            ...
            elif statement == "over":  # special case OVER without newline
                split_rec = [  # update list
                    re.sub(rf"\b({statement})\b", statement.upper(), line, flags=re.I)
                    if not re.search("(?:--.*|\/\*.*\*\/)", line)
                    else line
                    for line in split_rec
                ]
...
# Cell
def format_select_partition_by(s, indentation=7):
    "Format statement line `s` with `indentation` spaces"
    ...
    s = re.sub(r"(,)(\s*)([\w\d]+)", r"\1\n" + " " * indentation + r"\3", s)  # add newline after each comma (no comments) and indentation
    s = re.sub(r"\[EOC\]", "\n" + " " * indentation, s)  # replace [EOC] by newline
...
def format_select(s):
    ...
    return format_select_partition_by(s, 7)

# Cell
def format_partition_by(s):
    "Format partition by line `s`"
    return format_select_partition_by(s, 13)
...
def format_statement_line(s):
    ...
    statement_funcs = {
        ...
        "on": format_on,
        "where": format_where,
        "partition by": format_partition_by
    }
...
def format_simple_sql(s, add_semicolon=True):
    ...
    s = preformat_statement(s, MAIN_STATEMENTS)  # add breaklines for the main statements
...
def extract_outer_subquery(s):
    ...
    for i, c in enumerate(s):
        if (s[i:(i+8)] == "(\nSELECT" or s[i:(i+14)] == "(\nPARTITION BY") and ind: # query start
...
def format_subquery(s, previous_s):
    ...
    s = re.sub(r"^\(\nPARTITION BY", "(PARTITION BY", s)  # remove newline between parenthesis and PARTITION BY
    # get reference line for the indentation level
    # and remove whitespaces from the left
    last_line = previous_s.split("\n")[-1]
    if re.search("(?:FROM|JOIN)", last_line):  # if the last line is a FROM or JOIN statement
        ref_line = last_line.lstrip()
    else:  # else it is a PARTITION BY
        ref_line = last_line
    ...
    indented_s = [
        ...
        if not re.match(r"SELECT", line) and not re.match("PARTITION BY", line)
        else line
